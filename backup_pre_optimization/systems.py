...
def _turrets_attack(game_state, dt):
    ...
    for turret in game_state.buildings:
        if turret.type != BLDG_TURRET:
            continue
        # 冷却
        if not hasattr(turret, '_cooldown'):
...
def _update_repair(game_state, dt):
    ...
    if not hasattr(game_state, '_repair_accum'):
        game_state._repair_accum = {}
        game_state._repair_disp_accum = {}
        game_state._repair_disp_timer = {}
    for human in game_state.humans:
