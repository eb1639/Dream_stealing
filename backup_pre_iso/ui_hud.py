...
def render_pause_menu(screen, game_state, font_medium, font_btn):
    ...
    toggle_btn_rect = pygame.Rect(btn_x, panel_y + 55, btn_w, btn_h)
    exit_btn_rect = pygame.Rect(btn_x, panel_y + 110, btn_w, btn_h)

    # 根据当前暂停状态显示不同按钮文字
    toggle_label = "继续游戏" if game_state.paused else "暂停游戏"
    toggle_base_color = (40, 130, 60) if game_state.paused else (50, 80, 160)
    toggle_hover_color = (60, 170, 90) if game_state.paused else (70, 110, 210)

    for rect, label, base_color, hover_color in [
        (toggle_btn_rect, toggle_label, toggle_base_color, toggle_hover_color),
...
def handle_pause_menu_click(game_state, mouse_pos):
    ...
    if rects is None:
        return False
    if rects['toggle'].collidepoint(mouse_pos):
        # 动态按钮: 当前已暂停则继续(关闭菜单), 当前未暂停则暂停(菜单保持)
